                        ...
                        <div class="flex flex-wrap">
                            <div class="grid grid-cols-6 w-full gap-2">
                                ...
                                <div class="col-end-7 col-span-2 ...">
                                    <div class="w-full px-3 mb-6">
                                        <input class="input-recherche shadow-box" type="search" placeholder="Recherche" v-model="searchFORMATION">
                                    </div>
                                </div>
                            </div>
                        </div>
                            ...
                            <table class="shadow-box w-full rounded-lg">
                                <thead>
                                    <tr class="inter font-semibold text-xl border-b my-3 text-theme-bleu-marine">
                                        <th scope="col">Site & Département</th>
                                ...
                                <tbody>
                                    <tr v-for="item in filtered" :key="item">
                                        <td>{{ item.departement }} - {{ item.site }}</td>
                                        <td>{{ item.parcours }}</td>
                                        <td>{{ item.niveau }}</td>
                                        <td>{{ item.mention }}</td>
                                        ...
                                        <td>
                                            <div class="px-6 py-4 flex justify-center items-stretch text-gray-900">
                                                ...
                                                <span role="button" v-on:click="deleteFormation(item.id)">
        ...
        computed: {
            filtered: function() {
                ...
                search = search.filter(function(item) {
                    if (item.departement.toLowerCase().indexOf(searchFORMATION) !== -1 || item.departement.toUpperCase().indexOf(searchFORMATION) !== -1) {
                        return item;
                    }
                })
                return search;
