                        ...
                        <div class="flex flex-wrap">
                            <div class="grid grid-cols-6 w-full gap-2">
                                ...
                                <div class="col-end-7 col-span-2">
                                    <div class="w-full px-3 mb-6">
                                        <input class="input-recherche shadow-box md:p-2 sm:p-2 xl:p-32 lg:p-32 ml-28" type="search" placeholder="Recherche" v-model="searchComposante">
                                    </div>
                                </div>
                            </div>
                        </div>
                            ...
                            <table class="shadow-box w-full rounded-lg">
                                ...
                                <tbody>
                                    <tr v-for="item in composante" :key="item">
                                        <td>{{ item.libelle }}</td>
                                        <div class="px-6 py-4 flex items-stretch text-gray-900">
                                            ...
                                            <div class="text-sm text-gray-900">
                                                <span role="button" v-on:click="deleteComposante(item.id)">
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            download: function () {
                const data = XLSX.utils.json_to_sheet(this.composante);
                const wb = XLSX.utils.book_new();
                XLSX.utils.book_append_sheet(wb, data, "data");
                XLSX.writeFile(wb, "composante.xlsx");
            },
        ...
        computed: {
            filtered: function() {
                ...
                const searchComposante = this.searchComposante;
                if (!searchComposante) {
                    return search;
                }
                search = search.filter(function(item) {
                    if (item.libelle.toLowerCase().indexOf(searchComposante) !== -1 || item.libelle.toUpperCase().indexOf(searchComposante) !== -1) {
                        return item;
                    }
                })
                return search;
...
<style scoped lang="postcss">
    ...
    tbody tr td {
        @apply py-2 px-20 font-medium text-left;
    }
    caption {
        @apply opacity-0;
    }
