                        ...
                        <div class="flex flex-wrap">
                            <div class="grid grid-cols-6 w-full gap-2">
                                ...
                                <div class="col-end-7 col-span-2">
                                    <div class="w-full px-3 mb-6">
                                        <input class="input-recherche shadow-box md:p-2 sm:p-2 xl:p-32 lg:p-32 ml-28" type="search" placeholder="Recherche" v-model="searchDomaine">
                                    </div>
                                </div>
                            </div>
                        </div>
                                    ...
                                    <tr v-for="item in filtered" :key="item">
                                        ...
                                        <td>
                                            <div class="px-6 py-4 flex justify-center items-stretch text-gray-900">
                                                ...
                                                <span role="button" v-on:click="deleteDomaine(item.id)">
        ...
        methods: {
            getDomaine: function() {
                http
                    .get('domaines')
                    .then(res => {
                        this.domaine = res.data['hydra:member']
            ...
            deleteDomaine: function(id) {
                http
                    ...
                    .then(function(response) {
                        ...
                        if (this.stat === 204) {
                            this.getDomaine()
                            this.$toast.success(`Domaine supprimée avec succès`, {
                                position: "top-right"
                            })
            ...
            download: function() {
                const data = XLSX.utils.json_to_sheet(this.domaine);
                const wb = XLSX.utils.book_new();
                XLSX.utils.book_append_sheet(wb, data, "data");
                XLSX.writeFile(wb, "domaine.xlsx");
            },
        },
        computed: {
            filtered: function() {
