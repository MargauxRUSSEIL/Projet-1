                                    ...
                                    <tr v-for="item in filtered" :key="item">
                                        ...
                                        <td>
                                            <div class="px-6 py-4 flex justify-center items-stretch text-gray-900">
                                                <router-link :to="{ name: 'updateFormationModal', params: { id: item.id }}">
                                                ...
                                                <span role="button" v-on:click="deleteModaliteFormation(item.id)">
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            download: function() {
                const data = XLSX.utils.json_to_sheet(this.formationModal);
                const wb = XLSX.utils.book_new();
                XLSX.utils.book_append_sheet(wb, data, "data");
                XLSX.writeFile(wb, "ModaliteFormation.xlsx");
            },
            modifModaliteFormation: function(id) {
                this.$router.push({
                    name: 'update​FormationModal',
                    params: {
                        id: id
                    }
                })
