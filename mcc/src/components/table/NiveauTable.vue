                    ...
                    <div v-else>
                        <div class="grid grid-cols-6 w-full gap-2">
                            ...
                            <div class="col-end-7 col-span-2 ...">
                                <div class="w-full px-3 mb-6">
                                    <input class="input-recherche shadow-box md:p-2 sm:p-2 xl:p-32 lg:p-32 ml-28" type="search" placeholder="Recherche" v-model="searchNiveau">
                                </div>
                            </div>
                        </div>
                    <div>
                                ...
                                <tr v-for="item in filtered" :key="item">
                                    ...
                                    <td>
                                        <div class="px-6 py-4 flex justify-center items-stretch text-gray-900">
                                            ...
                                            <span role="button" v-on:click="deleteNiveau(item.id)">
        ...
        methods: {
            getNiveau: function() {
                http
                    .get('niveaux')
                    .then(res => {
                        this.niveau = res.data['hydra:member']
            ...
            deleteNiveau: function(id) {
                http
                    ...
                    .then(function(response) {
                        ...
                        if (this.stat === 204) {
                            this.getNiveau()
                            this.$toast.success(`Niveau supprimée avec succès`, {
                                position: "top-right"
                            })
        ...
        computed: {
            filtered: function() {
                ...
                search = search.filter(function(item) {
                    if (item.libelle.toLowerCase().indexOf(searchNiveau) !== -1 || item.libelle.toUpperCase().indexOf(searchNiveau) !== -1) {
                        return item;
                    }
                })
                return search;
